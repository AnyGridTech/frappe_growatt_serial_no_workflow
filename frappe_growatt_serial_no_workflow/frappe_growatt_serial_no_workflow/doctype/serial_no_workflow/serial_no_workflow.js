...
(() => {
  ...
  var OUTPUT_INFO_MESSAGE = {
    ...
    COMPANY_NOT_SELECTED: __("Company not selected."),
    // pt-BR: Seleção de Company requerida.
    ...
    SN_CREATED_SUCCESSFULLY: __("SN created successfully."),
    // pt-BR: SN criado com sucesso.
    SN_UPDATED_SUCCESSFULLY: __("SN updated successfully."),
    // pt-BR: SN atualizado com sucesso.
    PROCESSING_SNS: __("Processing Serial Numbers..."),
    // pt-BR: Processando Números de Série...
    PROCESS_COMPLETED: __("Process Completed"),
    // pt-BR: Processo Concluído
    PROCESS_FINISHED_SUCCESS: __("All serial numbers were processed successfully!")
    // pt-BR: Todos os números de série foram processados com sucesso!
  };
  ...
  async function CheckSerialNumber(sn) {
    let printError = "";
    if (!sn) return { item: void 0, snInfo: void 0, printError: "Empty SN.", growattModel: void 0 };
    let snInfo;
    try {
      snInfo = await get_sn_info(sn);
    } catch (e) {
      printError = "Error fetching SN info.";
      return { item: void 0, snInfo: void 0, printError, growattModel: void 0 };
    }
    if (!snInfo || Object.keys(snInfo).length === 0) {
      ...
      if (!sn2 || !sn2.data || !sn2.data.model) return { item: void 0, snInfo: void 0, printError, growattModel: void 0 };
      let item;
      try {
      ...
      return { item, snInfo: void 0, printError, growattModel: sn2.data.model };
    }
    return { item: {}, snInfo, printError: "", growattModel: void 0 };
  }
  ...
  async function validateAndAddToForm(form, sn_workflow, serialNumber, selectedState) {
    ...
    if (!agt?.utils?.validate_serial_number?.(serialNumber)) {
      ...
      try {
        const { item, snInfo: snInfo2, printError, growattModel } = await CheckSerialNumber(serialNumber);
        if (printError) {
          ...
          const companyOptions = companies && companies.length > 0 ? companies.map((c) => c.name) : [];
          const itemsWithMPPT = itemList.filter((i) => i.mppt != null);
          ...
          if (hasMPPT && selectedValues.mppt) {
            selectedItem = itemsWithMPPT.find((i) => String(i.mppt).trim() === String(selectedValues.mppt).trim());
      ...
      for (const op of operations) {
        ...
        if (op.isNew) {
          try {
            ...
            if (newSN) {
              form.doc.serial_no_table.filter((c) => c.serial_no === op.sn).forEach((c) => {
                c.is_success = 1;
  ...
  frappe.ui.form.on("Serial No Workflow", {
    refresh: async function(form) {
      ...
      try {
        ...
        let next_step_options = sn_workflow.states.map((s) => s.state).filter((state) => allowedStatesSet.has(state));
        const service_type = form.doc["service_type"];
        if (service_type === "Replacement") {
          next_step_options = next_step_options.filter((state) => state.startsWith("[AR]"));
        } else if (service_type === "Repair") {
          next_step_options = next_step_options.filter((state) => state.startsWith("[DR]"));
        }
...
${data.result.text}` : data.result.text;
    ...
    service_type: async function(form) {
      try {
